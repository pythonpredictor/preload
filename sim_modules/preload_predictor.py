...
class Preload(SimModule):
    ...
    def preload(self, event):
        ...
        current_hour = self.simulator.get_current_time(hour)
        self.index = current_hour/self.interval_time

        #appends the corresponding number of indicies to the freq_count_list
        for x in range(intervals):
            freq_count_list.append({})
        ...
        #before preloading the app check to see if it is morning, afternoon or night and then preload the corresponding application

                #for current time : simulator.get_current_time
    ...
    # for each app event in the frequency table, need to add a time_expon factor where it is decremented at an logorithmic rate
    # when verify is called every app in the corresponding freq_count table will be logged except for the app that is called
